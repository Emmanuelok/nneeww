  ...
  if (j.rules.payTransparency.required) {
    ...
    if (!found) {
      results.push({
        ...
        detail:
          "Ontario postings must disclose expected compensation or a range, unless the upper end exceeds $200,000.",
        citation: "ESA, s. 8.1 · Pay transparency in postings",
        suggestion:
          "Add a clearly stated range, e.g. “Expected compensation: $85,000–$115,000 CAD.” Keep the spread at or under $50,000.",
      });
      ...
      results.push({
        ...
        citation: "ESA, s. 8.1(3) · Above-threshold exemption",
      });
      ...
      results.push({
        ...
        citation: "ESA, s. 8.1(2) · Maximum range spread",
        suggestion:
          "Narrow the range to a maximum spread of $50,000. If you must keep it wider, split the posting by seniority.",
      });
    } else {
      results.push({
        id: "pay_range",
        label: "Pay transparency",
        status: "pass",
        message: `Range $${found.min.toLocaleString(
          "en-CA"
        )} – $${found.max.toLocaleString("en-CA")} is within the ${formatMoney(
          maxSpread ?? 0
        )} spread cap.`,
        citation: "ESA, s. 8.1 · Pay transparency in postings",
      });
  ...
  if (j.rules.vacancyDisclosure.required) {
    ...
    if (declared || found) {
      ...
      results.push({
        ...
        citation: "ESA, s. 8.2 · Existing-vacancy disclosure",
      });
    } else {
      results.push({
        id: "vacancy_disclosure",
  ...
  if (j.rules.prohibitedClauses.length > 0) {
    if (prohibited.length > 0) {
      ...
      results.push({
        ...
        label: "Prohibited language",
        status: "pass",
        message: "No prohibited 'Canadian experience' language detected.",
        citation: "ESA, s. 8.3 · Prohibited information requests",
      });
    }
  }
